...
\begin{example}
  ...
  Then, $\alpha^{(1)} \sqcup \alpha^{(2)} = \alpha^{(3)}$ and $\alpha \sqcap \alpha^{(5)} = \alpha^{(4)}$, as depicted in \Cref{fig:join-meet}.
  \begin{center}
    \begin{tikzpicture}[
      node/.style={draw=none, circle},
      cross/.style={midway, draw=none, fill=green!5, circle, inner sep=0pt, minimum size=8pt},
    ...
    \end{tikzpicture}
  \end{center}
  \captionof{figure}{Join and meet inside the lattice of \Cref{fig:loosening-hasse}, each computed as an extremum of an intersection of adjoint intervals (framed). Left: the join $\alpha^{(1)} \sqcup \alpha^{(2)} = \alpha^{(3)}$ is the least element of $[\alpha^{(1)},\mgf{\alpha^{(1)}}] \cap [\alpha^{(2)},\mgf{\alpha^{(2)}}]$. Right: the meet $\alpha \sqcap \alpha^{(5)} = \alpha^{(4)}$ is the greatest element of $[\lgf{\alpha},\alpha] \cap [\lgf{\alpha},\alpha^{(5)}]$.}%
  \label{fig:join-meet}
\end{example}
...
\[
(\mathbb{c}_\alpha,\mathbb{c}_\beta) : \castpath{\alpha \timesS \beta}{\alpha' \timesS \beta'}
\]
...
Every cast path $\mathbb{c} : \castpath{\alpha}{\beta}$ induces a \emph{semantic cast}: a ZFC function $\zop{\mathbb{c}} \in {\interpZ{\beta}}^{\interpZ{\alpha}}$ that sends each constructor to the canonical set-theoretic coercion between the interpretations of its source and target. Formally it is \leancodeptr{castZF\_of\_path}{SMT/Reasoning/LooseningDefs.html\#castZF_of_path}, defined by recursion on $\mathbb{c}$; it is the semantic shadow of $\mathsf{loosen}_{\mathbb{c}}$, in that where the construction rewrites a \emph{term}, the semantic cast transports the \emph{set} that term denotes. Throughout, $\mathbb{c}_\alpha : \castpath{\alpha}{\alpha'}$ and $\mathbb{c}_\beta : \castpath{\beta}{\beta'}$ denote the immediate sub-derivations, with $c_\alpha \triangleq \zop{\mathbb{c}_\alpha}$ and $c_\beta \triangleq \zop{\mathbb{c}_\beta}$ their semantic casts. The recursion has one clause per constructor.

\textbf{\texttt{refl}.} A reflexive path acts as the identity, $\zop{\mathbb{c}} \triangleq \idZ{\interpZ{\alpha}}$.

\textbf{\texttt{opt}.} The option cast loosens the payload constructor-wise:
\[
  \noneZ \;\mapsto\; \noneZ, \qquad \someZ\ a \;\mapsto\; \someZ\ \appZ c_\alpha(a).
\]

\textbf{\texttt{pair}.} The pair cast is the product of the component casts:
\[
  (a, b) \;\mapsto\; \bigl(\appZ c_\alpha(a),\; \appZ c_\beta(b)\bigr).
\]

\textbf{\texttt{chpred}.} The characteristic-predicate cast transports a predicate $X \in \mathbb{B}^{\interpZ{\alpha}}$ along $c_\alpha$:
\[
  X^! \;=\;
  \begin{cases}
    X \circ c_\alpha^{-1} & \text{on } \Ran(c_\alpha),\\
    \botZ & \text{off } \Ran(c_\alpha).
  \end{cases}
\]

\textbf{\texttt{fun}.} For $\beta \neq \boolS$, the function cast conjugates $X \in {\interpZ{\beta}}^{\interpZ{\alpha}}$ by the component casts:
\[
  X^! \;=\;
  \begin{cases}
    c_\beta \circ X \circ c_\alpha^{-1} & \text{on } \Ran(c_\alpha),\\
    d_{\beta'} & \text{off } \Ran(c_\alpha),
  \end{cases}
\]
where $d_{\beta'} \in \interpZ{\beta'}$ is the canonical default of $\interpZ{\beta'}$.

\textbf{\texttt{graph}.} The graph cast $\graphcastZ{c_\alpha}{c_\beta}$ carries a partial function $F \in (\optZ\,\interpZ{\beta})^{\interpZ{\alpha}}$ to the characteristic predicate of its graph, pushed forward through the two component casts.

The first five clauses are \emph{structural}: they loosen componentwise and leave the shape of the denoted value untouched, mirroring the homomorphic rules of $\sqsubseteq$. Only \ref{rule:castable-graph} changes shape, trading the functional encoding $\alpha \toS \optS\,\beta$ of a partial function for the relational encoding $(\alpha' \timesS \beta') \toS \boolS$ of its graph; its set-theoretic content---the only one not immediate from the constructor---is spelled out in \Cref{def:semantic-graph-cast} alongside the \texttt{graph} case.

Each $\zop{\mathbb{c}}$ is injective---the formal counterpart of the statement that loosening changes the \emph{representation} of a value, never the value itself.
...
\begin{proof}
By induction on $\mathbb{c}$. The \texttt{refl} case is the identity. The structural cases preserve injectivity componentwise; in the \texttt{chpred}, \texttt{fun}, and \texttt{graph} cases the pre-image reconstructions used above---$c_\alpha^{-1}$ and the pointwise inverses of the graph pushforward---are well-defined precisely because the component casts are injective by the inductive hypothesis.
\end{proof}

We write the value $b \in \interpZ{\beta}$ of $\zop{\mathbb{c}}$ at $a \in \interpZ{\alpha}$ either as $b = \appZ \zop{\mathbb{c}}(a)$ or as $(a, b) \in \zop{\mathbb{c}}$.
...
\subsection{Correctness of loosening}
...
\begin{remark}
  The actual statement in Lean is slightly stronger than the statement below: it also threads fresh-name generation, context extension, weakening, and totality of denotation under valuation updates.
...
\begin{tikzpicture}[
  ...
  \begin{scope}
    ...
    \node (spec) at (6.2,-2.1) {$\denval{\Phi}[\boolS][\irrelprf]$};
...
\end{tikzpicture}
\end{center}
\caption{The loosening square. Solid arrows are the construction $\mathrm{loosen}_{\mathbb{c}}$ and the denotation $\interpS{\aterm{\cdot}{\Delta}}$; the dashed arrow is the semantic cast $\zop{\mathbb{c}}$. Interpreting $\langle\lvar{x},\spec{\lvar{x}}\rangle$ under $\Delta' = \Delta[\lvar{x}\mapsto\denval{X^!}]$ recovers the cast image $X^!$ of $X$, and the specification evaluates to a boolean $\denval{\Phi}$. The square depicts the witness case (item~2), where $X^! = \appZ\zop{\mathbb{c}}(X)$ and $\Phi = \topZ$; at another typed target $\lvar{x}\mapsto\denval{Y}$ the specification is still defined, but $\Phi$ need not be $\topZ$.}
\label{fig:loosening-square}
\end{figure}

\begin{note}
\Cref{fig:loosening-square} summarizes the statement: the construction commutes with denotation and the semantic cast. This is deliberately weaker than a full semantic equivalence, and is exactly the invariant the recursion maintains. The soundness half of item~3 is what lets a recursive call conclude that its loosened sub-term equals the intended cast image; the \texttt{chpred} and \texttt{fun} cases rely on it most---\texttt{chpred} through the recursively generated specification of a fresh source variable, and \texttt{fun} at the level of the whole equation, where soundness sharpens into the pointwise characterization proved below.
\end{note}
...
\[
  ...
  \stackrel{\eqref{eq:refl_fresh}}{=} \denval{X = Y}[\boolS],
\]
where $X = Y$ is the (set-level, hence Lean-level) equality of the two denotations; it denotes $\topZ$ exactly when $Y = X$. Since $\appZ \zop{\mathbb{c}}(X) = X$, taking $Y = X^! = X$ gives item~2, while the displayed equivalence is precisely the definedness and soundness of item~3. Here the loosening square (\Cref{fig:loosening-square}) degenerates, its semantic cast collapsing to the identity, as \Cref{fig:refl-square} shows.
...
\begin{tikzpicture}[
  ...
  \node (X)    at (0,-2.1)   {$\denval{X}[\alpha][\irrelprf]$};
  \node (X!)   at (3.6,-2.1) {$\denval{Y}[\alpha][\irrelprf]$};
  \node (spec) at (6,-2.1) {$\denval{X = Y}[\boolS][\irrelprf]$};
...
\end{tikzpicture}
\end{center}
\captionof{figure}{The loosening square (\Cref{fig:loosening-square}) for \texttt{refl}: the semantic cast is the identity $\idZ{\interpZ{\alpha}}$, so $X^! = X$ and $\spec{\lvar{x}}$ reduces to $\lvar{x} \eqS x$. Under $\Delta' = \Delta[\lvar{x}\mapsto\denval{Y}]$ its value is the equality $X = Y$, which is $\topZ$ exactly when $Y = X$.}%
\label{fig:refl-square}
\end{proof}
...
\end{equation}
Because $\zop{\mathbb{c}_\alpha}$ and $\zop{\mathbb{c}_\beta}$ are injective, the witnesses $x$ and $y$ are unique whenever they exist. In short, the graph cast pushes the graph of $F$ through the two component casts.
\end{definition}
...
\end{equation}
Injectivity of the component casts makes the inverses well-defined on their ranges, so $X^! \in \interpZ{(\alpha' \timesS \beta') \toS \boolS}$; comparing with~(\ref{eq:cast-graphz-def}), $\appZ \graphcastZ{\zop{\mathbb{c}_\alpha}}{\zop{\mathbb{c}_\beta}}(X) = X^!$ by construction---the graph pushforward of \Cref{def:semantic-graph-cast}.
...
\end{equation}
Define $X^! : \interpZ{\alpha'} \to \mathbb{B}$ by $X^!(b) = X(a_0)$ when $b = \appZ c(a_0)$ for some $a_0$, and $X^!(b) = \mathsf{false}$ otherwise; injectivity of $c$ makes this well-defined, and $\appZ \zop{\mathbb{c}}(X) = X^!$ by definition of the characteristic-predicate cast, which transports $X$ along $c$.
...
\begin{proof}
Fix $\Delta$ with $\interpS{\aterm{x}{\Delta}} = \denval{X}$, so $\tau_X = \alpha \toS \beta$. With $d_{\beta'}$ the canonical default of $\interpZ{\beta'}$, the semantic function cast conjugates $X$ by the component casts:
\[
X^!(a') \triangleq
\begin{cases}
