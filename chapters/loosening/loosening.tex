...
\[
\spec{\lvar{x}} \;\;\triangleq\;\; \lvar{x} \eqS \lamS \lvar{z} : \alpha' \mapsto \existsS \var{z} : \alpha\cdot\, \appS x (\var{z}) \andS \spec{\lvar{z}}
\]
%
...
\begin{equation}
  ...
  \zop{\mathbb{c}} \triangleq \idZ{\interpZ{\alpha}}. \tag{$\zop{\mathtt{refl}}$}
\end{equation}

\paragraph{\texttt{opt}.} The option cast loosens the value under $\someS$ constructor-wise; from \(\mathbb{c} : \castpath{\optS\ \alpha}{\optS\ \alpha'}\), we obtain \(\mathbb{c}_\alpha : \castpath{\alpha}{\alpha'}\) and define
\begin{equation}
  \label{eq:castZF-opt}
...
\paragraph{\texttt{pair}.} The pair cast is the product of the component casts; from \(\mathbb{c} : \castpath{\alpha \timesS \beta}{\alpha' \timesS \beta'}\), we obtain \(\mathbb{c}_\alpha : \castpath{\alpha}{\alpha'}\) and \(\mathbb{c}_\beta : \castpath{\beta}{\beta'}\) and define
\begin{equation}
...
\paragraph{\texttt{chpred}.} The characteristic-predicate cast transports predicates along casts; from \(\mathbb{c} : \castpath{(\alpha \toS \boolS)}{(\alpha' \toS \boolS)}\), we obtain \(\mathbb{c}_\alpha : \castpath{\alpha}{\alpha'}\) and define
\begin{equation}
  \label{eq:castZF-chpred}
  \zop{\mathbb{c}} \;\triangleq\;
  \lamZ\,X \mapsto \left(\lamZ\,b \mapsto
  \begin{cases}
    \appZ X\bigl(\appZ {\zop{\mathbb{c}}}_\alpha^{-1}(b)\bigr) & \text{if } b \in \Ran(\zop{\mathbb{c}}_\alpha),\\
    \botZ & \text{otherwise}
  \end{cases}\right).
  \tag{$\zop{\mathtt{chpred}}$}
\end{equation}

\paragraph{\texttt{fun}.} The function cast conjugates a function by the component casts; from \(\mathbb{c} : \castpath{(\alpha \toS \beta)}{(\alpha' \toS \beta')}\) with $\beta \neq \boolS$, we obtain \(\mathbb{c}_\alpha : \castpath{\alpha}{\alpha'}\) and \(\mathbb{c}_\beta : \castpath{\beta}{\beta'}\) and define
\begin{equation}
  \label{eq:castZF-fun}
  \zop{\mathbb{c}} \;\triangleq\;
  ...
  \tag{$\zop{\mathtt{fun}}$}
\end{equation}
where $d_{\beta'} \coloneq \interpS{\mathsf{default}(\beta')} \in \interpZ{\beta'}$ is the canonical default of $\interpZ{\beta'}$ (\Cref{def:smt-default}).

\paragraph{\texttt{graph}.} The graph cast carries a partial function $F \in (\optZ\,\interpZ{\beta})^{\interpZ{\alpha}}$ to the characteristic predicate of its graph, pushed forward through the two component casts:
\begin{equation}
...
Item~2 is a completeness statement---the cast image $\appZ \zop{\mathbb{c}}(X)$ satisfies $\spec{\lvar{x}}$---and item~3 a soundness one---among typed targets, only the cast image does. Together they show that $\spec{\lvar{x}}$ has a unique typed solution, namely $\appZ \zop{\mathbb{c}}(X)$; and since $\zop{\mathbb{c}}$ is injective, this image in turn determines $X$, which is the precise sense in which $\lvar{x}$ ``is'' the loosened $x$.
...
Throughout, we do not construct the proof component of denotation triples, often writing $\irrelprf$ instead: building these witnesses---as the Lean development does at every step---would only burden already heavy arguments, and they are routinely recovered from the local context. Their explicit constructions can be found in the Lean proofs.
...
\end{tikzpicture}
\end{center}
\caption{The loosening square. Solid arrows depict assumptions, dashed arrows denote conclusions, and $\Delta' \coloneq \Delta[\lvar{x}\mapsto\denval{X^!}[\beta]]$.}
...
Taking $Y = X^! = X$ therefore reduces this equality to \(\topZ\), which is item~2, while the displayed equivalence is precisely the definedness and soundness of item~3. Here the loosening square (\Cref{fig:loosening-square}) degenerates, its semantic cast collapsing to the identity, as shown below.
...
\begin{description}
  ...
  \item[If $x = \someS\ y\,$:] the specification is
    ...
    \begin{fact}
    The loosened specification $\spec{\lvar{x}}$ is satisfied at the cast image, \ie under $\Delta_{\lvar{x}}$,
    \[
      \interpS{\aterm{\spec{\lvar{x}}}{\Delta_{\lvar{x}}}} = \denval{\topZ}[\boolS].
    \]
    \end{fact}
    \begin{subproof}
    Recall that $\existsS \lvar{y}:\alpha'\cdot\ \psi$ abbreviates $\notS (\forallS \lvar{y}:\alpha'\cdot\ \notS\psi)$ (\Cref{def:smt-denote}), and that $\forallS$ is denoted by the infimum (in the sense of the smallest set, obtained by an intersection) over $\interpZ{\alpha'}$ of its body values. Since an infimum in $\mathsf{ZFBool}$ is $\botZ$ as soon as any of its values is, it suffices to exhibit a witness $V \in \interpZ{\alpha'}$ for the bound variable $\lvar{y}$ under which the body $\psi \coloneq (\lvar{x} \eqS \someS\ \lvar{y}) \andS \spec{\lvar{y}}$ is denoted by $\topZ$; at that witness $\notS\psi$ is denoted by $\botZ$, so
    \[
      \interpS{\aterm{\forallS \lvar{y}:\alpha'\cdot\ \notS\psi}{\Delta_{\lvar{x}}}} = \botZ,
    \] hence $\interpS{\aterm{\spec{\lvar{x}}}{\Delta_{\lvar{x}}}} = \notZ\botZ = \topZ$.
    
    Taking $Y^!$ for the witness $V$ and $\Delta'' \coloneq \Delta_{\lvar{x}}[\lvar{y} \mapsto \denval{Y^!}[\alpha']]$, the two conjuncts of $\psi$ are such that:
    \[
      \interpS{\aterm{\lvar{x} \eqS \someS\ \lvar{y}}{\Delta''}} \;=\; (\someZ\ Y^! \eqZ \someZ\ Y^!) \;=\; \topZ
      \quad\text{and}\quad
      \interpS{\aterm{\spec{\lvar{y}}}{\Delta''}} = \topZ,
    \]
    the first because $\lvar{x}$ and $\someS\ \lvar{y}$ are both denoted by $\someZ\ Y^!$ under $\Delta''$, the second because $\Delta''$ agrees with $\Delta'$ on the free variables of $\spec{\lvar{y}}$ (as $\lvar{x}$ does not occur in $\spec{\lvar{y}}$). Thus, $\interpS{\aterm{\psi}{\Delta''}} = \topZ \andZ \topZ = \topZ$, which establishes the fact.
    \end{subproof}
  \item[Otherwise:] the definition emits the dynamic test $\iteS (x \eqS \noneS_{\alpha})\,(\cdots)\,(\cdots)$: if $X = \noneZ$ it reduces to the first case, else $\tau_X = \optS\,\alpha$ forces $X = \someZ\ Y_0$, and
  \[
    \interpS{\aterm{\theS x}{\Delta''}} = \denval{Y_0}[\alpha],
  \] reducing to the second with $X^! = \someZ\ Y_0^!$.
\end{description}
...
Here the loosening square (\Cref{fig:loosening-square}) specializes to the $\someS$ constructor: its semantic cast carries $\someZ\ Y$ to $\someZ\ Y^!$, as shown above.
\end{proof}
...
\begin{fact}
The loosened pair-specification $\spec{\lvar{x}}$ is satisfied at the cast image, \ie under $\Delta_{\lvar{x}} \coloneq \Delta[\lvar{x} \mapsto \denval{X^!}[\alpha' \timesS \beta']]$,
\[
  \interpS{\aterm{\spec{\lvar{x}}}{\Delta_{\lvar{x}}}} = \denval{\topZ}[\boolS].
\]
\end{fact}
...
Here the loosening square (\Cref{fig:loosening-square}) specializes to the product, its semantic cast $\zop{\mathbb{c}} = (\zop{\mathbb{c}_\alpha}, \zop{\mathbb{c}_\beta})$ acting componentwise:
...
Assume $\stype{\Gamma}{x}{\alpha \toS \boolS}$, let $\mathbb{c}_\alpha : \castpath{\alpha}{\alpha'}$ and let $\mathbb{c} \coloneq \texttt{chpred}\,\mathbb{c}_\alpha : \castpath{(\alpha \toS \boolS)}{(\alpha' \toS \boolS)}$. \Cref{thm:loosening-correctness} holds with target type $\alpha' \toS \boolS$ and semantic cast $\mathbb{c}$ \eqref{eq:castZF-chpred}.
\end{lemma}
...
\end{fact}
\begin{subproof}
Apply \Cref{thm:loosening-correctness} inductively\footnotemark{} to the fresh source variable $\var{z} : \alpha$, well typed with $\interpS{\aterm{\var{z}}{\Delta_{\var{z}, \var{z}^!}}} = \denval{Z}[\alpha]$. Item~3 gives that $\spec{\lvar{z}}$ is defined at every typed target $b$ and, by soundness, the forward implication: if $\Phi_{Z,Z^!} = \topZ$ then $Z^! = \appZ \zop{\mathbb{c}}_\alpha(Z)$. Item~2 gives the converse: at the cast image $Z^! = \appZ \zop{\mathbb{c}}_\alpha(Z)$ the specification holds, so $\Phi_{Z,\appZ \zop{\mathbb{c}}_\alpha(Z)} = \topZ$. Together they yield \eqref{eq:zspec_chpred}.
\end{subproof}
...
\[
  \spec{\lvar{x}} \;\;\triangleq\;\; \lvar{x} \eqS \left(\lamS \lvar{z}:\alpha' \mapsto \existsS \var{z}:\alpha\cdot\ \appS x(\var{z}) \andS \spec{\lvar{z}}\right),
\] whose $\lamS$-body denotes $X^!$ pointwise.

\begin{fact}
For every $b \in \interpZ{\alpha'}$, writing $\Delta_{\lvar{z}} \coloneq \Delta[\lvar{z} \mapsto \denval{b}[\alpha']]$---the source variable $\var{z}$ is bound by the inner existential, hence left unassigned,
\[
  \interpS{\aterm{\existsS \var{z}:\alpha\cdot\ \appS x(\var{z}) \andS \spec{\lvar{z}}}{\Delta_{\lvar{z}}}} = X^!(b).
\]
\end{fact}
\begin{subproof}
Two cases on $b$:
\begin{description}
  \item[if $b \notin \Ran(\zop{\mathbb{c}}_\alpha)$ then:] by \eqref{eq:zspec_chpred}, \(\spec{\lvar{z}}\) is \(\botZ\) at $(a, b)$ for every $a$, \ie
    \[
    \forall a \in \interpZ{\alpha},\quad
    \interpS{\aterm{\spec{\lvar{z}}}{\Delta_{\lvar{z}}[\var{z} \mapsto \denval{a}[\alpha]]}} = \denval{\botZ}[\boolS],
    \]
    a true value would force $\appZ \zop{\mathbb{c}}_\alpha(a) = b \in \Ran(\zop{\mathbb{c}}_\alpha)$.
    So, each disjunct $\appS x(\var{z}) \andS \spec{\lvar{z}}$ is (denoted by) $\botZ$ and the existential is $\botZ = X^!(b)$.
    
  \item[if $b = \appZ \zop{\mathbb{c}}_\alpha(a_0)$ then:] the existential is $\topZ$ if and only if there exists an $a$ such that $\appZ X(a) = \topZ$ and $\spec{\lvar{z}}$ true at $(a, b)$. By \eqref{eq:zspec_chpred}, the latter forces $\appZ \zop{\mathbb{c}}_\alpha(a) = b = \appZ \zop{\mathbb{c}}_\alpha(a_0)$, hence $a = a_0$ by injectivity of $\zop{\mathbb{c}}_\alpha$. Thus, the existential is $\topZ$ exactly when $\appZ X(a_0) = \topZ$, that is, exactly when $\appZ X(a_0) = X^!(b)$.
\end{description}
\end{subproof}
...
For item~3, at any typed target $\denval{Y}[\alpha' \toS \boolS]$ the $\lamS$-body ($\appS x(\var{z})$ has a denotation, so has $\spec{\lvar{x}}$. It is therefore \(\topZ\) at $Y$ if and only if $Y$ equals the body's denotation $\appZ \zop{\mathbb{c}}(X)$, \ie if and only if $Y = X^!$.

The loosening square (\Cref{fig:loosening-square}) here transports a predicate along $\zop{\mathbb{c}}_\alpha$, its semantic cast relabelling the domain by $b = \appZ \zop{\mathbb{c}}_\alpha(a)$ and defaulting to $\botZ$ off $\Ran(\zop{\mathbb{c}}_\alpha)$:
...
\end{proof}
\footnotetext{
  The six cases detailed here as distinct sections are the actual cases in the proof of \Cref{thm:loosening-correctness}, carried out by induction on cast paths. A recursive call to \Cref{thm:loosening-correctness} is therefore legitimate.
}
...
\]
well-defined since $\zop{\mathbb{c}}_\alpha$ is injective, hence invertible on its range.
\end{minipage}\hfill
...
\[
  ...
  D = d_{\gamma},
\]
where $d_{\gamma} \coloneq \interpS{\mathsf{default}(\gamma)} \in \interpZ{\gamma}$.
\end{fact}
\begin{subproof}
By structural induction on $\gamma$, following the definition of $\mathsf{defaultSpec}$ (\Cref{case:fun}).
Constant (\(\intS, \boolS, \unitS\)) are trivial.

For $\gamma = \optS\,\xi$, $\interpS{\aterm{t \eqS \noneS}{\Delta}}$ is $\topZ$ exactly when $D = \noneZ = d_{\optS\,\xi}$.

For $\gamma = \xi \timesS \kappa$, the predicate is the conjunction
\[
  \mathsf{defaultSpec}_{\xi}(\fstS t) \andS \mathsf{defaultSpec}_{\kappa}(\sndS t).
\]
By the induction hypotheses, we obtain $\fstZ D = d_{\xi}$ and $\sndZ D = d_{\kappa}$, \ie
\[
  D = (d_{\xi}, d_{\kappa}) = d_{\xi \timesS \kappa}.
\]

For $\gamma = \xi \toS \kappa$, the predicate is
\[\forallS \var{u}:\xi\cdot\ \mathsf{defaultSpec}_{\kappa}(\appS t(\var{u})).\]
By the induction hypothesis, the predicate evaluates to $\topZ$ at each $\var{u}$ under \(\var{u} \mapsto \denval{u}[\xi]\) exactly when $\appZ D(u) = d_{\kappa}$, hence overall exactly when $D$ is the constant map $d_{\xi \toS \kappa}$.
\end{subproof}
...
The loosening square (\Cref{fig:loosening-square}) specializes with the function conjugation as its semantic cast:
...
Assume $\stype{\Gamma}{x}{\alpha \toS \optS\,\beta}$, and let $\mathbb{c}_\alpha : \castpath{\alpha}{\alpha'}$ and $\mathbb{c}_\beta : \castpath{\beta}{\beta'}$. Then \Cref{thm:loosening-correctness} holds with target type $(\alpha' \timesS \beta') \toS \boolS$ and semantic cast \eqref{eq:castZF-graph}.
\end{lemma}

\begin{proof}
Fix a valuation $\Delta$ and a set $X$ with $\interpS{\aterm{x}{\Delta}} = \denval{X}$. By typing, $X \in (\optZ\,\interpZ{\beta})^{\interpZ{\alpha}}$ is a partial function. The \texttt{graph} case (\Cref{case:graph}) loosens a fresh source pair $\var{z} : \alpha \timesS \beta$ through the pair cast $(\mathbb{c}_\alpha, \mathbb{c}_\beta)$, yielding $\langle \lvar{z}, \spec{\lvar{z}} \rangle$ whose specification characterises the pair cast.

\begin{fact}
For $u=(a,b) \in \interpZ{\alpha \timesS \beta}$ and $v=(a',b') \in \interpZ{\alpha' \timesS \beta'}$, the denotation $\Phi_{u,v}$ of $\spec{\lvar{z}}$ under $\var{z} \mapsto \denval{u}$, $\lvar{z} \mapsto \denval{v}$ is defined, and
...
\end{fact}
\begin{subproof}
The recursive call is $\mathsf{loosen}_{(\mathbb{c}_\alpha,\mathbb{c}_\beta)}(\var{z})$, so the \texttt{pair} case (\Cref{lem:loosening-pair}) applies. Its item~3 gives definedness of $\spec{\lvar{z}}$ and, by soundness, the forward implication: a typed $v$ with $\Phi_{u,v} = \topZ$ equals the pair-cast image of $u$, whose components are $\appZ \zop{\mathbb{c}_\alpha}(a)$ and $\appZ \zop{\mathbb{c}_\beta}(b)$. Its item~2 gives the converse, that $\spec{\lvar{z}}$ holds at that cast image. Together they yield \eqref{eq:zspec_graph}.
\end{subproof}
...
\begin{equation}\label{eq:x!_graph_def}
  ...
  \appZ X\big(\appZ {\zop{\mathbb{c}_\alpha}}^{-1}(a')\big)=\someZ\ \appZ {\zop{\mathbb{c}_\beta}}^{-1}(b'). \tag{\ensuremath{\star}}
\end{equation}
...
The \texttt{graph} case defines $\spec{\lvar{x}} \triangleq \lvar{x} \eqS \lamS \lvar{z}:\alpha' \timesS \beta' \mapsto \existsS \var{z}:\alpha \timesS \beta\cdot\ (\appS x(\fstS\var{z}) \eqS \someS(\sndS\var{z})) \andS \spec{\lvar{z}}$, whose $\lamS$-body denotes $X^!$ pointwise.

\begin{fact}
For every $v=(a',b') \in \interpZ{\alpha' \timesS \beta'}$, the existential body denotes $X^!(v)$:
\[
  \interpS{\aterm{\existsS \var{z}:\alpha \timesS \beta\cdot\ (\appS x(\fstS\var{z}) \eqS \someS(\sndS\var{z})) \andS \spec{\lvar{z}}}{\cdot[\lvar{z} \mapsto \denval{v}]}} = X^!(v).
\]
\end{fact}
\begin{subproof}
If $a' \notin \Ran(\zop{\mathbb{c}_\alpha})$ or $b' \notin \Ran(\zop{\mathbb{c}_\beta})$, the right-hand side of \eqref{eq:zspec_graph} fails for every $u$, so $\Phi_{u,v} \neq \topZ$ throughout; the existential is $\botZ$, matching $X^!(v) = \botZ$. Otherwise let $a = {\zop{\mathbb{c}_\alpha}}^{-1}(a')$ and $b = {\zop{\mathbb{c}_\beta}}^{-1}(b')$ be the unique pre-images. The existential body at $v$ is $\topZ$ if and only if $\appZ X(a)=\someZ\ b$. Indeed, if $\appZ X(a)=\someZ\ b$ then $u=(a,b)$ is a witness: its first conjunct holds because $\fstZ u = a$, $\sndZ u = b$, and $\appZ X(a)=\someZ\ b$, and $\spec{\lvar{z}}$ holds at $(u,v)$ by \eqref{eq:zspec_graph}. Conversely, any witness $u=(a_0,b_0)$ gives $\appZ X(a_0)=\someZ\ b_0$ from the first conjunct and $\zop{\mathbb{c}_\alpha}(a_0)=a',\ \zop{\mathbb{c}_\beta}(b_0)=b'$ from \eqref{eq:zspec_graph}, whence $a_0=a$ and $b_0=b$ by injectivity. Thus the body is $\topZ$ exactly on the condition \eqref{eq:x!_graph_def}, \ie it denotes $X^!(v)$.
\end{subproof}

Consequently the outer equality $\spec{\lvar{x}}$ holds under $\lvar{x} \mapsto \denval{X^!}[(\alpha' \timesS \beta') \toS \boolS]$, which is item~2. For item~3, at any typed target $\denval{Y}$ with $\tau_Y = (\alpha' \timesS \beta') \toS \boolS$ the $\lamS$-body is defined on typed data---the application, the $\someS$, and the recursive $\spec{\lvar{z}}$ are all defined---so $\spec{\lvar{x}}$ is defined; and being the equality $\lvar{x} \eqS \lamS(\cdots)$, if it holds at $Y$ then $Y$ equals the $\lamS$-body's denotation $X^! = \appZ \graphcastZ{\zop{\mathbb{c}_\alpha}}{\zop{\mathbb{c}_\beta}}(X)$, that is $(X,Y)$ lies in the graph cast.

This is the one shape-changing rule: the loosening square (\Cref{fig:loosening-square}) turns the functional encoding of a partial function into the relational encoding of its graph.
...
\noindent Concretely, the graph cast $\zop{\mathbb{c}} = \graphcastZ{\zop{\mathbb{c}_\alpha}}{\zop{\mathbb{c}_\beta}}$ sends the partial function $X$ (an arrow diagram, with undefined points sent to $\noneZ$) to the characteristic predicate of its pushed-forward graph: a single $\topZ$ at each pair $(\appZ\zop{\mathbb{c}_\alpha}(a), \appZ\zop{\mathbb{c}_\beta}(b))$ with $\appZ X(a) = \someZ\,b$, and $\botZ$ everywhere else---in particular off the ranges of the component casts.

\begin{center}
\begin{tikzpicture}[>=stealth, dot/.style={circle, fill, inner sep=1.1pt}]
  \node at (0,2.1) {$X:\alpha \rightharpoonup \beta$};
  \draw[rounded corners] (-0.5,-1.6) rectangle (0.5,1.6);
  \draw[rounded corners] (2.0,-1.6) rectangle (3.0,1.6);
  \node at (0,-1.95) {$\interpZ{\alpha}$};
  \node at (2.5,-1.95) {$\optZ\,\interpZ{\beta}$};
  \node[dot,label=left:{$a_1$}] (a1) at (0,1.0) {};
  \node[dot,label=left:{$a_2$}] (a2) at (0,0.0) {};
  \node[dot,label=left:{$a_3$}] (a3) at (0,-1.0) {};
  \node[dot,label=right:{$\someZ b_1$}] (s1) at (2.5,1.0) {};
  \node[dot,label=right:{$\someZ b_2$}] (s2) at (2.5,0.0) {};
  \node[dot,label=right:{$\noneZ$}] (n)  at (2.5,-1.0) {};
  \draw[relarr,draw=black] (a1) -- (s1);
  \draw[relarr,draw=black] (a2) -- (s2);
  \draw[relarr,draw=black] (a3) -- (n);
  \draw[funarr] (3.15,1.95) to[bend left=20] node[midway,above]{$\graphcastZ{\zop{\mathbb{c}_\alpha}}{\zop{\mathbb{c}_\beta}}$} (5.9,1.9);
  \begin{scope}[shift={(5.9,-1.5)}]
    \draw[->] (0,0) -- (3.4,0) node[right]{$\alpha'$};
    \draw[->] (0,0) -- (0,3.4) node[above]{$\beta'$};
    \fill[black!8] (0,0) rectangle (2.0,2.0);
    \draw (0,0) rectangle (2.0,2.0);
    \node[below] at (1.0,-0.1) {$\Ran(\zop{\mathbb{c}_\alpha})$};
    \node[rotate=90, anchor=south] at (-0.12,1.0) {$\Ran(\zop{\mathbb{c}_\beta})$};
    \fill (0.6,1.4) circle (2pt); \node[above right, inner sep=1pt] at (0.6,1.4) {\scriptsize$\topZ$};
    \fill (1.4,0.6) circle (2pt); \node[above right, inner sep=1pt] at (1.4,0.6) {\scriptsize$\topZ$};
    \node[align=left, anchor=west] at (2.35,1.4) {\footnotesize $\botZ$\\ elsewhere};
  \end{scope}
\end{tikzpicture}
\end{center}
\end{proof}