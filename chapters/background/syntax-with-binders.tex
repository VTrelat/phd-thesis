...
The contrast is made concrete in \Cref{ex:foas-hoas}, where the identity abstraction \(\lambda x ↦ x\) is rendered both as deep first-order syntax and through a shallow binder.
...
\label{ex:foas-hoas}
The object-language identity abstraction\footnotemark \(\lambda x ↦ x\) can be contrasted as follows:
\[
\begin{array}{@{}c@{\qquad}c@{}}
  \text{FOAS} & \text{HOAS} \\[0.3em]
  \mathsf{lam}\ \mathtt{x}\ (\mathsf{var}\ \mathtt{x})
    & \mathsf{lam}\ (\pink{\lambda} x ↦ x)
\end{array}
\]
On the left, the body is already a syntax tree and the bound occurrence is represented by the name \(\texttt{x} : \mathsf{Name}\).
On the right, the displayed \(\pink{\lambda}\) is a meta-language binder: the body is obtained by applying the resulting function to the syntactic representative \(x : \mathsf{Term}\) of the bound variable.
\end{example}

\footnotetext{{The symbol \(\pink{\lambda}\) is deliberately reused for object-language abstraction and for meta-language functions; which is meant is always clear from context, and keeping the two notations distinct would only burden the presentation.}}
...
For each value \(d : \mathcal D\), instantiating the body yields the term \(P(d) : \mathsf{Term}\ \mathcal D\), whose denotation \(\left\lBrack P(d) \right\rBrack\) is computed recursively.
The denotation of \(\int P\), denoted \(\lBrack \int P \rBrack\) or \(\left\lBrack \int \left( \pink\lambda d \mapsto P(d)\right) \right\rBrack\), is then defined from the meta-level function
\(
  \pink\lambda d \mapsto \left\lBrack P(d) \right\rBrack
\)
---for instance as a quantifier, a supremum, or a similar aggregate over \(d\), according to the binder \(\int\).
...
The denotation may still be partial, and each object-language constructor still requires its own semantic clause, together with the lemmas that establish the clause's intended behaviour.
This will be clearer in \Cref{ch:b} and \Cref{ch:smt}, where denotational semantics are defined.
