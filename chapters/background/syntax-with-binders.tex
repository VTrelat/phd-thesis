...
For this reason, \emph{parametric higher-order abstract syntax} (PHOAS) will be an important
...
This is the distinction between \emph{deep} and \emph{shallow} embedding.

A representation is deep for a given construct when occurrences of that construct are retained as object-language syntax.
For example, in a first-order abstract syntax (FOAS) representation with named variables, application, and lambda abstraction may be represented by constructors of the following respective shape:
\[
\begin{array}{rcl}
...
Higher-order abstract syntax (HOAS) changes the representation of binders by replacing the first-order body syntax with a meta-level function,
...
Many proof assistants and assisted theorem-proving systems that provide inductive datatypes enforce a strict-positivity condition on inductive definitions.
...
\end{example}
Thus ordinary HOAS requires either a meta-language designed for HOAS, as in logical-framework presentations of HOAS~\cite{pfenning1988hoas}, or extra restrictions and adequacy arguments before it can be used as a faithful inductive representation of syntax.
...
In this work, PHOAS is not just prettier syntax for binders; parametricity is also used as a trick to define semantics. 
If we consider a semantic domain \(\mathcal D\) for the object language, then \(\mathsf{Term}\ \mathcal D\) is the type of syntax whose variables already are semantic values, not names.
...
The denotation has the schematic shape of a \emph{partial} function from syntax to semantics:
\[
  \lBrack \cdot \rBrack :
    \mathsf{Term}\ \mathcal D \rightharpoonup \mathcal D.
\]
...
Higher-order syntax is used where reasoning would otherwise be obscured by administrative overhead about names.
