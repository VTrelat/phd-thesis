...
abbreviations
\[
  x \regnotation{\subseteq} y \;\;\coloneq\;\; \forall z,\, z \in x \to z \in y,
  \qquad
...
and~(\ref{ax:found}) we also use the standard abbreviations $\regnotation{\varnothing}$
(the empty set), $\regnotation{\{y\}}$ (the singleton set containing $y$), $\regnotation{x \cup y}$ (the binary
union of $x$ and $y$), and $\regnotation{x \cap y}$ (the binary intersection of $x$ and $y$),
each of which unfolds into a formula over $\in$ alone, but their use improves
readability and reflects the way the axioms are applied in practice. The axioms
of ZF are the following eight; the ninth, choice, stands apart and is presented
in \Cref{subsec:set-theory-choice}.
...
\begin{enumerate}[label=\textbf{(A\arabic*)}, ref=A\arabic*, leftmargin=3.4em, itemsep=0.8ex]
  ...
  \]
  \begin{note}
    Such a set \(y\) is unique by extensionality, and is denoted \(\regnotation{\setcomp{z \in x}{\varphi(z, \vec p\,)}}\).
  \end{note}
  ...
  \begin{note}
    ...
    \(\varphi\); in this case the set \(b\) is denoted \(\regnotation{\setcomp{\varphi(x)}{x \in a}}\).
  \end{note}
\end{enumerate}
