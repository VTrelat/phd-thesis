...
\paragraph*{Mizar and Lisa}
Mizar~\cite{mizar-sota} is a long-standing system whose logic is first-order with an object-level axiomatization of Tarski--Grothendieck set theory; its large Mizar Mathematical Library is developed entirely at the set level, with a ``soft'' type system layered on top to recover type-like conveniences.
More recently, Lisa~\cite{lisa-itp2023} builds a proof assistant on a kernel for schematic first-order logic, on top of which axiomatic set theory is developed, with proofs and tactics written in a Scala-embedded language.
Both keep mathematics at the set level, as we do; our emphasis differs in that we deliberately interoperate with a dependently typed host (Lean and Mathlib) through explicit bridges, reusing its tactics, type classes, and algebraic hierarchy rather than developing the set-level layer in isolation.
