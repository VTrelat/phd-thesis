...
Further usual operations such as cartesian product (\(\regnotation{\times}\)), union (\(\regnotation{\cup}\)), and powerset (\(\regnotation{\mathcal{P}}\)), are defined on pre-sets, all to be later lifted to sets.
...
\begin{definition}[][ZFSet.π₁, ZFSet.π₂][ZFLean/Integers.html, ZFLean/Integers.html]
  ...
  \begin{equation*}
    ...
    \begin{cases}
      \pi_1\, x                                           & \text{if}\ \left(\bigcup x\right) \setminus \bigcap x = \varnothing \\
      \bigcup \left( \left(\bigcup x\right) \setminus \bigcap x\right) & \text{otherwise}
    \end{cases}
  \end{equation*}
\end{definition}
