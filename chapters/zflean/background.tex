...
These structures are called \emph{pre}-sets, rather than sets, because they do not yet carry an extensional notion of identity: the same collection may be described by many distinct pre-sets, as illustrated by \Cref{ex:pset-ext}.
...
\begin{example}
  \label{ex:pset-ext}
  Two pre-sets with the same extension are not necessarily equal.
  Given two pre-sets \(X \eqcolon \langle \alpha, A \rangle\) and \(Y \eqcolon \langle \beta, B \rangle\), one cannot prove that \(X = Y\) from \(X \sim Y\), \ie in Lean, the following is not provable:
  \begin{leanlisting}
example {X Y : PSet} (h : ∀ x, x ∈ X ↔ x ∈ Y) : X = Y -- unprovable
  \end{leanlisting}
  It is however sufficient that the functions \(A\) and \(B\) are extensionally equivalent to prove \(X = Y\):
  \begin{leanlisting}
variable (α : Type u) (A : α → PSet.{u}) (B : α → PSet.{u})

example (h : ∀ x, A x = B x) : (⟨α, A⟩ : PSet) = ⟨α, B⟩ := by
  congr      -- apply congruence on the constructor
  ext x      -- apply functional extensionality
  exact h x  -- apply the hypothesis
\end{leanlisting}
\end{example}
...
With these projections, we can prove the expected simplification lemmas for ordered pairs, e.g.\ \(\pi_1\, (x, y) = x\) and \(\pi_2\, (x, y) = y\) for any sets \(x\) and \(y\), \(\eta\)-expansion of pairs and membership on a cartesian product, as shown in \cref{lst:pair-proj} (see {\small\leancodeptr{π₁\_pair}{ZFLean/Integers.html\#ZFSet.π₁_pair}}, {\small\leancodeptr{π₂\_pair}{ZFLean/Integers.html\#ZFSet.π₂_pair}}, {\small\leancodeptr{pair\_eta}{ZFLean/Integers.html\#ZFSet.pair_eta}}, {\small\leancodeptr{pair\_mem\_prod}{Mathlib/SetTheory/ZFC/Basic.html\#ZFSet.pair_mem_prod}} in the development)---with proofs omitted.
