...
\end{theorem}
%
Proving \Cref{thm:isIso_curry} requires a few definitions and lemmas, which we introduce first.
...
\]
Concretely, these two facts are registered with the \leaninl{zfun} attribute as theorems {\small\leancodeptr{currify\_is\_func}{ZFLean/Isomorphisms.html\#ZFSet.currify_is_func}} and {\small\leancodeptr{uncurrify\_is\_func}{ZFLean/Isomorphisms.html\#ZFSet.uncurrify_is_func}}: whenever a later side condition of the form ``$\zunop{\mathsf{curry}}$ (resp.\ $\zunop{\mathsf{uncurry}}$) is a total function'' is generated---by an application, a composition, or an extensionality step---the \inlinelean|zfun| tactic locates the tagged lemma and discharges it automatically, without the user restating it.
...
\begin{proof}
  ...
  \end{equation*}
  The right-hand side reduces to $f$ by definition of the identity function and using the following theorem {\small\leancodeptr{fapply\_Id}{ZFLean/Functions.html\#ZFSet.fapply_Id}}:
  \begin{leanlisting}
...
The proof of \cref{thm:isIso_curry} then boils down to applying lemma {\small\leancodeptr{isIso\_of\_two\_sided\_inverse}{ZFLean/Isomorphisms.html\#ZFSet.isIso_of_two_sided_inverse}} with the equalities from \cref{lem:curry-uncurry-inverse}:
\begin{leanlisting}
