...
\begin{theorem}[Currying isomorphism]
...
%
\begin{definition}[Currying and uncurrying]
  ...
  \end{equation*}
  %
  We prove that $\zunop{\mathsf{curry}}$ and $\zunop{\mathsf{uncurry}}$ are total functions on their respective domains, so that automation can pick those facts up when needed:
  $\mathsf{IsFunc}(C^{A \times B}, (C^B)^A, \zunop{\mathsf{curry}})$ and $\mathsf{IsFunc}((C^B)^A, C^{A \times B}, \zunop{\mathsf{uncurry}})$.
\end{definition}
...
\end{lemma}
\begin{proof}
  We sketch the proof of the first equality; the second one follows a similar pattern.
  Full proof details are available in the artifacts.
...
theorem fapply_Id {A x : ZFSet} (hx : x ∈ A) : @ᶻ𝟙A ⟨x, $\irrelprf$⟩ = ⟨x, hx⟩ := $\irrelprf$
\end{lstlisting}
...
theorem fapply_composition {g f : ZFSet} {A B C : ZFSet}
  ...
  @ᶻ(g ∘ᶻ f) ⟨x, $\irrelprf$⟩ = @ᶻg ⟨@ᶻf ⟨x, $\irrelprf$⟩, $\irrelprf$⟩ := $\irrelprf$
\end{lstlisting}
...
The proof of \cref{thm:isIso_curry} then boils down to applying the following theorem with the equalities from \cref{lem:curry-uncurry-inverse}:
\begin{lstlisting}[language=lean]
theorem isIso_of_two_sided_inverse {A B : ZFSet} {f g : ZFSet}
  {hf : A.IsFunc B f} {hg : B.IsFunc A g}
  (left_inv : g ∘ᶻ f = 𝟙A) (right_inv : f ∘ᶻ g = 𝟙B) : A ≅ᶻ B := $\irrelprf$
\end{lstlisting}

\begin{lstlisting}[language=lean, float=t,
  caption={Proof skeleton of the currying isomorphism.},
...
\subsection{Size of the artifact}
...
\begin{table}[t]
  ...
  \begin{tabular}{lrr}
    ...
    \hline
    \textbf{Total}                               & \textbf{8,324} & \textbf{706}        \\
    \hline
  \end{tabular}
\end{table}

The artifact is a self-contained Lean~4 library layered on top of Mathlib's ZFC model consisting of {8,324} lines of code (LoC), organized in a few modules summarized in \cref{tab:artifacts-loc}, and containing 706 proved theorems.
