...
Modern mathematical developments are increasingly carried out with proof assistants, whose foundational cores range from first-order set theories---as in Mizar~\cite{mizar-sota} or Isabelle/ZF~\cite{paulson-zf}---to more expressive systems based on simple type theory---as in Isabelle/HOL---or dependent type theory---as in Rocq~\cite{RocqReferenceManual}, Agda~\cite{AgdaReferenceManual}, and Lean~\cite{mathlib4}.
This evolution was fueled by the \emph{proofs-as-programs} vision of the Curry-Howard correspondence, and it crystallized into two broad and often contrasted families of foundations: \emph{collection}-based theories (set-theoretic) and \emph{construction}-based theories (type-theoretic).
Various proposals aim to reconcile these viewpoints, either by generalizing over them---\eg category theory---or by refining them---\eg constructive set theories such as CZF~\cite{aczel-czf}.

Nevertheless, formalizing \emph{set-level} mathematics is sometimes desirable---for extensional equalities (two sets defined by different comprehensions coinciding), partiality via relations (a partial map such as integer division or a finite lookup table), or category-style ``up to isomorphism'' reasoning (treating $\mathbb{Z}$ or $\mathbb{Q}$ as quotients).
