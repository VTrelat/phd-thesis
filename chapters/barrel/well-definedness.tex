...
One may return an arbitrary value outside the intended domain; the resulting theorems are
then true of the Lean definition but not of the B operator it is meant to denote, and
nothing in the statement records the discrepancy.
...
For application, the second field alone---membership of \(x\) in \(\domB(f)\), that is,
\(\existsB y \cdot (x,y) \inB f\)---already yields a value by choice; the first field,
...
Cardinality is the exception and is worth noting, because it shows that the proof argument
is not a mere guard: from \leaninl{S.Finite} the definition builds a \leaninl{Fintype}
...
writes, which is exactly the situation examined in \Cref{sec:barrel-atelier-comparison}.
