...
The encoding of partial operators departs from the B method's own treatment:
well-definedness evidence is turned into a proof argument, and \barrel{}
...
\begin{table}[t]
  ...
  \begin{tabular}{lll}
    ...
    \(\appB f\,x\)    & \leaninl{app}  & \(f \inB \domB(f) \tfunB \ranB(f)\) and \(x \inB \domB(f)\) \\
...
Atelier~B can emit well-definedness obligations, and does so on request.
...
\begin{leanlisting}[title={The two bridging theorems relating \barrel{}'s definedness
    conditions for the integer extrema to Atelier~B's.},
    label={lst:barrel-atelierb-bridge}, float=t]
...
\begin{remark}
  ...
  The library proves the two characterizations equivalent for the integer extrema
  (\Cref{lst:barrel-atelierb-bridge}), the only operators for which such a bridging theorem
  exists: {\small\barrelcodeptr{min.WD\_iff\_AtelierB\_WD}{Barrel/Builtins/Arithmetic.lean\#L707}}
  and {\small\barrelcodeptr{max.WD\_iff\_AtelierB\_WD}{Barrel/Builtins/Arithmetic.lean\#L757}}.
