...
\definecolor{barrelUiSorry}{RGB}{158,88,8}
...
\definecolor{barrelUiLegendSorry}{RGB}{190,122,42}
...
\begin{figure}[t]
  ...
  \caption{Both bars are updated in
    real time: green is for the goals the automation has already discharged, pale blue is for those
    parsed but not yet decided, and the gray remainder is for those still to be parsed. A gray
...
\begin{figure}[t]
  ...
  \caption{Two states that a source file alone does not distinguish. In (a) the goals have
    ...
    their goals, so the action disappears while the goals are marked as admitted and
    excluded from the proved count.}
