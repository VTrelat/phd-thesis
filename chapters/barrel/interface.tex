...
Two things are invisible while a development is being built.
...
It is strictly observational---nothing in the construction or the checking of a theorem
depends on it, and it can be switched off---so it belongs to the comfort of the workflow and
not to the trust argument of \Cref{subsec:barrel-automation}.
...
works through the file, so an import can be watched rather than waited out.
In \Cref{fig:barrel-state-loading}, \texttt{MinSearch} is done at six of seven goals and
\texttt{MinSearch\_r1} is still being imported.
...
\begin{figure}[t]
  ...
  \caption{The panel while \texttt{MinSearch\_r1} is being imported. Both bars are updated in
    real time: the pale blue part of a bar is the goals parsed so far, whose outcome is not
    yet known, and the green part those the automation has already discharged. A gray status
    disc marks an artifact whose final count is still to come.}
...
The distinction matters, since a syntactically complete development otherwise looks
finished---though the guarantee rests on the axiom check of
\Cref{sec:barrel-atelier-comparison} and not on a color.
...
the import statistics---for \texttt{MinSearch\_r2}, twenty-six of forty-seven distinct goals
discharged automatically and seventy-four repeated definedness conditions resolved by reuse
...
\begin{figure}[ht!]
  ...
  \caption{Admission-free completion of the refinement chain: the three artifacts collapsed
    (top) and the completed \texttt{MinSearch\_r2} card expanded (bottom), with its goal map
