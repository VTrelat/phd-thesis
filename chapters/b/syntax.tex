...
\begin{definition}[Additional concrete terms][B.Term, String.toBinaryOp][B/Syntax/Extra.html, POGReader/POGReader.html\#String.toBinaryOp]
  ...
  primitive syntax of \Cref{def:b-basic-syntax}.

  \textbf{Logical connectives:}
  \begin{align*}
    x \notinB y                     & \triangleq \notB(x \inB y) \tag{negated membership}                                      \\
    ...
    x \bop{\leftrightarrow} y       & \triangleq \powB (x \prodB y) \tag{set of relations}
  \end{align*}
\end{definition}
...
\begin{definition}[B types][B.BType][B/Typing/Basic.html\#B.BType]
  \label{def:b-types}
  \begin{bnf}
    \leaninl{B.BType} \(\,∋\, \tau\) ::= \(\intB\) && \(\boolB\) && \(\setB\tau\) && \(\tau \prodB \tau\)
  \end{bnf}
\end{definition}
...
directly available at the level of concrete syntax.
...
\begin{definition}[Set-theoretic constructs and operations][B.Builtins, toBinaryOp, toUnaryOp][
  ...
  \begin{align*}
    ...
    S \subseteqB T      & \triangleq \forallB v \inB S \cdot v \inB T                                           \\
    S \notsubseteqB T   & \triangleq \notB(S \subseteqB T)                                                      \\
    S \ssubB T          & \triangleq \{\,v \inB S \mid v \notinB T\,\}                                          \\
...
\begin{definition}[Finite subsets][B.Term.finite][POGReader/Builtins.html\#B.Term.finite]
  ...
  \begin{align*}
    \mathsf{FIN}(A)            & \triangleq \{\, S \inB \powB(A) \mid \existsB n \inB \ZintB \cdot \existsB f \inB S \injtfunB \ZintB \cdot \forallB x, y \inB S \prodB \ZintB \cdot \\
                               & \hspace{1cm}(x \mapletB y \inB f) \impB ((\mathsf{int}\ 0 \leB y) \andB (y \leB n)) \,\}                                                            \\
    {\mathsf{FIN}_1}_{\tau}(A) & \triangleq \{\, S \inB \mathsf{FIN}(A) \mid S \neqB \varnothing_{\tau} \,\}
  \end{align*}
...
\begin{definition}[Arithmetic constructs][B.Builtins, toBinaryOp][
  ...
  Let \(i, j, x, y\) be B terms.
  \begin{align*}
    \mathbb{N}       & \triangleq \{\, n \inB \ZintB \mid \mathsf{int}\ 0 \leB \mathsf{int}\ n \,\} \\
    \intrangeB{i}{j} & \triangleq \{\, n \inB \ZintB \mid i \leB n \andB n \leB j \,\}              \\
    x \geB y         & \triangleq y \leB x                                                          \\
    x \gtB y         & \triangleq \notB(x \leB y)                                                   \\
    x \ltB y         & \triangleq \notB(y \leB x)
  \end{align*}
\end{definition}
...
\begin{definition}[Domain restriction and subtraction][B.Term.domRestriction, B.Term.domSubtraction][
  ...
  \[
    F \domRestrB R \triangleq \{\, x, y \inB R \mid x \inB F \,\} \quad\text{and}\quad
    F \domSubtrB R \triangleq \{\, x, y \inB R \mid x \notinB F \,\}
  \]
\end{definition}
...
\begin{definition}[Range restriction and subtraction][B.Term.ranRestriction, B.Term.ranSubtraction][
  ...
  Let \(F\) and \(R\) be B terms.
  \[
    R \ranRestrB F \triangleq \{\, x, y \inB R \mid y \inB F \,\} \quad\text{and}\quad
    R \ranSubtrB F \triangleq \{\, x, y \inB R \mid y \notinB F \,\}
  \]
\end{definition}
...
\begin{definition}[Projections][toBinaryOp][
  ...
  \begin{align*}
    ...
    \mathsf{prj}_2(Q, R) & \triangleq \{\, x, y, z \inB Q \prodB R \prodB Q \mid z \eqB y \,\}
  \end{align*}
\end{definition}
