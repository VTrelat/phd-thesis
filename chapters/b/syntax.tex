...
from raw proof-obligation files and thereby provides a deep embedding of the B
language.
...
\begin{remark}
  ...
  such sets, along with enumerations, are represented as sets of integers, since
  they are by definition non-empty and finite.
...
\begin{example}
  Intersection \( t \capB s \) can be obtained from the bounded comprehension
  \[
    \{\,v \inB t \mid v \inB s\,\}.
  \]
  This definition captures the intended semantics of intersection; in
  particular, it yields an associative and commutative operation.
\end{example}
...
\begin{definition}[Set-theoretic constructs and operations][B.Builtins, toBinaryOp, toUnaryOp][
  ...
  Let \(S\) and \(T\) be B terms, and let \(\tau\) and \(\sigma\) be B types.
  For the empty set, the parser expects \(\tau\) to match a set type:
  \(\tau \equiv \setB(\alpha)\).
  For the non-empty powerset, the parser expects \(\tau\) to match a set-of-sets
  type: \(\tau \equiv \setB(\setB(\alpha))\).
  \begin{align*}
    \varnothing_{\tau}  & \triangleq \{\,v \inB \lift{\alpha} \mid \mathsf{bool}\ \mathsf{false}\,\}            \\
    ...
    {\powB_1}_{\tau}(S) & \triangleq \{\,v \inB \powB(S) \mid \existsB x \inB \lift{\alpha} \cdot x \inB v \,\}
  \end{align*}
\end{definition}
...
\begin{definition}[Finite subsets][B.Term.finite][POGReader/Builtins.html\#B.Term.finite]
  ...
  \end{align*}
  For non-empty finite subsets, the parser expects \(\tau\) to match a set type.
\end{definition}
...
\begin{definition}[Domain and range of a relation][B.Term.dom, B.Term.ran][
  ...
  \label{def:b-syntax-dom-ran}
      Let \(R\) be a B term, and let \(\tau\) be a B type.
  In both definitions, the parser expects \(\tau\) to match a relation type, \ie
  a set of pairs: \(\tau \equiv \setB (\alpha \prodB \beta)\).
  \begin{align*}
    \domB[\tau](R) & \triangleq \{\, x \inB \lift{\alpha} \mid \existsB y \inB \lift{\beta} \cdot (x \mapletB y) \inB F \,\} \\
    \ranB[\tau](R) & \triangleq \{\, y \inB \lift{\beta} \mid \existsB x \inB \lift{\alpha} \cdot (x \mapletB y) \inB F \,\}
  \end{align*}
\end{definition}
...
\begin{definition}[Overloading][B.Term.overload][
  ...
  Let \(Q\) and \(R\) be B terms, and let \(\tau\) be a B type.
  The parser expects \(\tau\) to match a relation type, \ie a set of pairs:
  \(\tau \equiv \setB (\alpha \prodB \beta)\).
  \[
...
\begin{definition}[Inverse relation][toUnaryOp][
  ...
  Let \(R\) be a B term, and let \(\tau\) be a B type.
  The parser expects \(\tau\) to match a relation type: \(\tau \equiv \setB (\alpha \prodB \beta)\).
  \[
    R^{-1}_{\tau} \triangleq \{\, x, y \inB \lift{\beta} \prodB \lift{\alpha} \mid y \mapletB x \inB R \,\}
  \]
\end{definition}
...
\begin{definition}[Direct image][String.toBinaryOp][
  ...
  Let \(R\) and \(S\) be B terms, and let \(\tau\) be a B type.
  The parser expects \(\tau\) to match a set type: \(\tau \equiv \setB (\alpha)\).
  \[
    R[S]_{\tau} \triangleq \{\, y \inB \lift{\alpha} \mid \existsB x \inB S \cdot x \mapletB y \inB R \,\}
  \]
\end{definition}
...
\begin{definition}[Direct product][toBinaryOp][
  ...
  Let \(Q\) and \(R\) be B terms, and let \(\tau\) be a B type.
  The parser expects \(\tau\) to match a relation type of the form
  \(\tau \equiv \setB(\alpha \prodB (\beta \prodB \gamma))\).
  \begin{align*}
...
\begin{definition}[Sequences][B.Term.seq, \etc][
  ...
  \begin{align*}
    \mathsf{seq}(A)    & \triangleq \{\, s \inB \mathbb{N} \pfunB A \mid \existsB n \inB \ZintB \cdot \existsB f \inB s \tfunB \intrangeB{\mathsf{int}\ 0}{n} \,\}               \\
    \mathsf{seq}_1(A)  & \triangleq \{\, s \inB \mathbb{N} \pfunB A \mid \left(\existsB n \inB \ZintB \cdot \existsB f \inB s \tfunB \intrangeB{\mathsf{int}\ 0}{n}\right) \andB \\
                       & \hspace{1cm} \existsB x, y \inB \mathbb{N} \prodB A \cdot \appB f\ x \eqB y \,\} \tag{non-empty}                                                        \\
...
\end{definition}
\inlremark[]{PHOAS does not provide constructors for min, max and cardinal.}
