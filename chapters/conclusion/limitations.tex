...
The formal B semantics covers the mathematical fragment \beer{} requires;
machines, substitutions, refinement, and proof-obligation generation lie outside
...
Coverage is a second limit: the verified \beer{} encoder leaves out, among
...
Both routes rest on a trusted base, and its principal component is the same:
proof-obligation generation. \beer{} additionally trusts the decoder of
Atelier~B proof-obligation files and its elaborations, the printer from abstract
scripts to concrete SMT-LIB, and the solver's unsatisfiability answer; the
composition of the term-level contracts into a theorem about a complete script
is a manuscript-level argument rather than a mechanized one. \barrel{}
additionally trusts the faithfulness of its shallow B-to-Lean embedding: the
kernel can reject an invalid derivation of a Lean proposition, but it cannot
establish that this proposition is the correct translation of the original B
obligation. Removing the shared component is the subject of the next section.
