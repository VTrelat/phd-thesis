...
The development deliberately mirrors that of the source language of \Cref{ch:b}, so that the two
...
model partiality.
The unit sort \(\unitS\) is a singleton sort, whose sole inhabitant is written \(\aop{\unitvalS}\) and
only appears in the abstract syntax since we never emit a bare unit literal in the concrete syntax.
...
\begin{note}
  The concrete syntax provides no constructor for the unit \emph{value}: although
  the sort \(\unitS\) exists, the encoder never emits a bare unit literal. The unit
  value therefore figures only in the abstract syntax (\Cref{def:smt-abstract-syntax},
  written \(\aop{\unitvalS}\)), where it serves as the default inhabitant of \(\unitS\)
  (\Cref{def:smt-default}).
\end{note}
