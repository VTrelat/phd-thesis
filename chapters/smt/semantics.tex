...
operations:
\[
  \interpS{t \aop{\addS} s} \triangleq \denval{X \addZ Y}[\intS],\quad
  \interpS{t \aop{\subS} s} \triangleq \denval{X \subZ Y}[\intS],\quad
  \interpS{t \aop{\mulS} s} \triangleq \denval{X \mulZ Y}[\intS],
\]
and comparison yields a Boolean: \(\interpS{t \aop{\leS} s} \triangleq \denval{X \leqZ Y}[\boolS]\).
\begin{proofconstruct}
...
\begin{align*}
  \interpS{t \aop{\eqS} s}  & \triangleq \denval{X = Y}[\boolS] && \text{when } \tau = \sigma, \\
  \interpS{t \aop{\andS} s} & \triangleq \denval{X \andZ Y}[\boolS] && \text{when } \tau = \sigma = \boolS, \\
...
\begin{proof}
  ...
  \paragraph{Variables and literals.}
  For a variable, type-compatibility supplies a domain value \(\Delta(\var{v})\) of
  type \(\tau\); abstraction maps \(\var{v}\) to it, variables are denoted by
  ...
  \(\varphi\) well-typed under \(\Gamma[\vec{\var{v}}:\vec\tau]\) for every family of variables
