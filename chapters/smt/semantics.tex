...
surprising: one might expect the same variable \(x\) to be typed differently under
different contexts, as in
\[
  \satype{\{ x \mapsto \intS\}}{\mathsf{var}\ x}{\intS} \quad\text{and}\quad
  \satype{\{ x \mapsto \boolS\}}{\mathsf{var}\ x}{\boolS},
\]
leaving the type carried by \(\interpS{\mathsf{var}\ x}\) ambiguous. The ambiguity is
only apparent. By definition \(\interpS{\mathsf{var}\ x} \triangleq x\), where \(x\)
...
\(x\) precisely this intrinsic type, so the two contexts above cannot both be
...
\begin{proof}
  ...
  type \(\tau\); abstraction maps \(\mathsf{var}\ v\) to it, variables are denoted by
  themselves. Literals and the unit value are immediate, with the membership
  ...
  \(\varphi\) well-typed under \(\Gamma[\vec v:\vec\tau]\) for every family of variables
  ...
  \smallskip\noindent\emph{Step 2: body denotation at typed tuples.} Each \(z\) induces a family
    ...
    \(\Delta' \coloneq \Delta[\vec v \leftarrow \vec z]\);
  ...
  \smallskip\noindent\emph{Step 4: assembling the witness.} Writing \(\Phi\) for
    ...
    \(\interpS{\varphi((\denval{z_i}[\tau_i][\irrelprf])_i)} = \denval{\Phi(z)}[\gamma][\irrelprf]\)
...
arises at every partial operator, \eg application, cardinality, minimum, and maximum,
