...
We write \(\regnotation{\coloneq}\) to denote \emph{definitional assignment}---binding a name to a term,
or giving the body of a definition---as distinct from propositional equality \(=\)
(a proposition asserting that two terms are equal) and from the metatheoretic symbol
\(\regnotation{\triangleq}\), read ``defined as'', used in displayed mathematics.

When the proof is unused or irrelevant in a given context, we simply write ``\(\regnotation{\irrelprf}\)'' in
