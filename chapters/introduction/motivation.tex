...
artificial intelligence (AI) assistants moved from short, relatively local
exchanges to sustained sessions over sizeable programs and formal developments,
...
machine-generated claims and artefacts.
...
question answering coexist with brittle behaviour on apparently simpler tasks
...
accompanies it. Fluency makes these cases harder, not easier, to distinguish.
As the cost of producing plausible answers falls, the scarce resource becomes a
justification that deserves confidence.
...
can provide an independent criterion for accepting AI-generated artefacts,
...
software correct.'' They provide languages and calculi in which artefacts,
assumptions, and arguments can be made precise. The artefact may be a program,
...
establish the wrong requirement; an axiom can be inappropriate; a model can omit
...
mechanism checked the derivation, and which components remain trusted?
...
This thesis adopts that separation as a guiding principle. Its ambition is to
construct a trustworthy reasoning chain in which increasingly powerful forms
of automation---including AI-assisted proof construction---can be used without
making the validity of a result depend on the opacity of the tool that proposed it.
A methodological account of how LLMs were themselves used during this work is given in
\Cref{sec:intro-llm-use}.

\subsection{Two modes of machine-assisted reasoning}
\label{subsec:intro-itp-atp}

Computer-assisted proof is commonly organized around two complementary kinds of
system. An \emph{automated theorem prover} (ATP) receives a conjecture and
searches for a proof with little or no guidance from its user. Modern
satisfiability modulo theories (SMT) solvers such as Z3~\cite{demoura2008z3} and cvc5~\cite{barbosa2022cvc5} are particularly
...
theories. Their main attraction is throughput: once an obligation has been
translated into the solver's language, large batches can be attempted automatically.
...
proofs which lie beyond the available automation require insight and
maintenance.
...
translation. The source and target languages---\eg B and SMT-LIB---may differ
...
The work in this thesis originates from this tension between \emph{automation}
and \emph{assurance}.
